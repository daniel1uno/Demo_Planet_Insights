...
if (!CLIENT_ID || !CLIENT_SECRET || !PL_API_KEY) {
  console.error('Please provide both SENTINEL CLIENT_ID, SENTINEL CLIENT_SECRET  and PL_API_KEY in the .env file');
  process.exit(1);
}
...
const server = createServer(async (req, res) => {
  ...
  // Routes to fetch informatiom from Planet
    ...
    try {
      const planetResponse = await fetch('https://api.planet.com/analytics/subscriptions/', {
        headers: {
      ...
      if (!planetResponse.ok) {
        throw new Error(`Failed to fetch data from Planet API: ${planetResponse.statusText}`);
      }
  ...
  else if (req.url.startsWith('/get-analytics-results') && req.method === 'GET') {
